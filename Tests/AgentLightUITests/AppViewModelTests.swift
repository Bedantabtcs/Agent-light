...
final class AppViewModelTests: XCTestCase {
    ...
    func testNewApprovalPendingRegistrationIsDurableMonitorsAndPromotesOnStatusRetry() async throws {
        ...
        harness.loginItem.currentStatus = .enabled
        await harness.viewModel.refreshLaunchAtLoginStatus()
    ...
    func testPendingApprovalPromotionFailureRemainsRetryableWithoutLoginMutation() async {
        let store = ControllableSetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        harness.loginItem.registerResult = .requiresApproval
        await harness.connectAndApprove()
        ...
        XCTAssertEqual(receipt?.login, .pendingApproval)
        XCTAssertEqual(harness.viewModel.phase, .repairRequired)
        XCTAssertEqual(harness.viewModel.presentedError, .operationFailed)
        XCTAssertTrue(harness.viewModel.loginStatusReconciliationRequired)
        XCTAssertTrue(harness.viewModel.outstandingObligations.contains(.ownershipReceiptRepair))
        ...
        XCTAssertEqual(harness.viewModel.phase, .monitoring)
        XCTAssertNil(harness.viewModel.presentedError)
        XCTAssertFalse(harness.viewModel.loginStatusReconciliationRequired)
        XCTAssertTrue(harness.viewModel.outstandingObligations.isEmpty)
        XCTAssertEqual(harness.loginItem.enableCount, counts.0)
        XCTAssertEqual(harness.loginItem.disableCount, counts.1)
    ...
    func testPendingApprovalPromotionRetryRestoresPausedWithoutLoginMutation() async {
        let store = ControllableSetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        harness.loginItem.registerResult = .requiresApproval
        await harness.connectAndApprove()
    ...
    func testLaunchAtLoginOptOutDisablesOwnedRegistrationWithoutRemovingSetup() async throws {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        let credentials = try XCTUnwrap(harness.credentials.storedCredentials())
        ...
        XCTAssertEqual(harness.credentials.storedCredentials(), credentials)
        XCTAssertTrue(harness.viewModel.monitoringActive)
    }

    func testLaunchAtLoginOptOutDisablesPendingApprovalAndCanRetryAfterFailure() async throws {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        harness.loginItem.registerResult = .requiresApproval
        await harness.connectAndApprove()
        harness.loginItem.disableError = HarnessSensitiveError("CANARY_LOGIN_DISABLE")
        ...
        let disabledReceipt = try await store.load()
        XCTAssertEqual(harness.viewModel.loginItemStatus, .notRegistered)
        XCTAssertEqual(disabledReceipt?.login, PersistentLoginOwnership.none)
    ...
    func testRelaunchDerivesDisabledLoginReceiptMismatchAndRetriesWithoutLoginMutation() async throws {
        ...
        XCTAssertFalse(relaunched.loginStatusReconciliationRequired)
    ...
    func testUnknownLoginMismatchFailsClosedWithoutClearingDurableAuthority() async throws {
        ...
        XCTAssertFalse(relaunched.loginDisabledStatePersistenceRetryRequired)
        XCTAssertTrue(relaunched.loginStatusReconciliationRequired)
        XCTAssertEqual(receipt?.login, .registered)
        XCTAssertEqual(harness.loginItem.disableCount, 0)
        XCTAssertEqual(harness.loginItem.enableCount, 1)
    }

    func testUnknownLoginStatusRetryResolvesToMonitoringWithoutLoginMutation() async throws {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.currentStatus = .unknown
        ...
        XCTAssertTrue(relaunched.loginStatusReconciliationRequired)
        harness.loginItem.currentStatus = .enabled
        await relaunched.refreshLaunchAtLoginStatus()
    ...
    func testRequiresApprovalStatusRetryResolvesToMonitoringWithoutLoginMutation() async throws {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.currentStatus = .requiresApproval
    ...
    func testUnknownStatusRefreshToAbsentOffersReceiptRetryWithoutLoginMutation() async throws {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.currentStatus = .unknown
    ...
    func testRequiresApprovalStatusRefreshToAbsentOffersReceiptRetryWithoutLoginMutation() async {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.currentStatus = .requiresApproval
    ...
    func testUnknownCompensationLaterEnabledClearsOnlyTransientRepairWithoutMutationOrWrite() async throws {
        let store = ControllableSetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        ...
        await store.failSaves([nextWrite])
        await harness.viewModel.setLaunchAtLoginEnabled(false)
        let writesAfterFailure = await store.writes()
        let counts = (harness.loginItem.enableCount, harness.loginItem.disableCount)
        XCTAssertTrue(harness.viewModel.outstandingObligations.contains(.ownershipReceiptRepair))
        ...
        let receipt = await store.current()
        let finalWrites = await store.writes()
        XCTAssertEqual(harness.viewModel.phase, .monitoring)
        XCTAssertNil(harness.viewModel.presentedError)
        XCTAssertTrue(harness.viewModel.outstandingObligations.isEmpty)
        XCTAssertEqual(receipt?.login, .registered)
        XCTAssertEqual(finalWrites, writesAfterFailure)
        XCTAssertEqual(harness.loginItem.enableCount, counts.0)
        XCTAssertEqual(harness.loginItem.disableCount, counts.1)
    }

    func testRequiresApprovalCompensationLaterEnabledRestoresPausedWithoutMutation() async {
        let store = ControllableSetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        await harness.viewModel.pause()
    ...
    func testCompensationBecomingAbsentRetainsTransientRepairUntilReceiptRetrySucceeds() async {
        let store = ControllableSetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.registerResult = .unknown
        ...
        let writesAfterRefresh = await store.writes()
        XCTAssertTrue(harness.viewModel.loginDisabledStatePersistenceRetryRequired)
        XCTAssertTrue(harness.viewModel.outstandingObligations.contains(.ownershipReceiptRepair))
        XCTAssertEqual(writesAfterRefresh, writesAfterFailure)
        XCTAssertEqual(harness.loginItem.enableCount, counts.0)
        XCTAssertEqual(harness.loginItem.disableCount, counts.1)
    ...
    func testAmbiguousLoginResolutionReturnsToPausedWithoutLoginMutation() async {
        let harness = ViewModelHarness()
        await harness.connectAndApprove()
        await harness.viewModel.pause()
        harness.loginItem.currentStatus = .unknown
    ...
    func testDisconnectAndNewSetupClearStaleLoginReconciliation() async {
        let store = MemorySetupOwnershipStore()
        let harness = ViewModelHarness(ownershipStore: store)
        await harness.connectAndApprove()
        harness.loginItem.currentStatus = .notRegistered
        let model = makeRelaunchedViewModel(using: harness, store: store)
        await model.synchronizeOwnership()
        XCTAssertTrue(model.loginDisabledStatePersistenceRetryRequired)

        await model.disconnect()

        XCTAssertEqual(model.phase, .onboarding)
        XCTAssertFalse(model.loginDisabledStatePersistenceRetryRequired)
        XCTAssertFalse(model.loginStatusReconciliationRequired)
        let reconnected = AppViewModel(
            credentials: harness.credentials,
            integrations: harness.integrations,
            monitor: harness.monitor,
            loginItem: harness.loginItem,
        ...
        await reconnected.connect(using: harness.validDraft)
        await reconnected.approveIntegrations()

        XCTAssertEqual(reconnected.phase, .monitoring)
        XCTAssertFalse(reconnected.loginDisabledStatePersistenceRetryRequired)
        XCTAssertFalse(reconnected.loginStatusReconciliationRequired)
    }
    ...
    func testRelaunchHydratesReceiptAndResumesWithoutReinstallingOrRewritingSetup() async throws {
        ...
        XCTAssertEqual(retainedReceipt, seededReceipt)
        XCTAssertEqual(relaunched.viewModel.maskedAccessID, "••••S_ID")
        XCTAssertEqual(relaunched.viewModel.maskedDeviceID, "••••E_ID")
        let verificationCount = await relaunched.verifier.count()
