...
final class MonitoringOrchestratorTests: XCTestCase {
    ...
    func testCompletedHoldExpiresFromPhysicalApplyWhileCommitPersistenceIsBlocked() async throws {
        let clock = ManualClock()
        let light = RecordingLightController(attemptClock: clock)
        let store = MemoryRecoveryStore()
        ...
        let timerScheduled = await eventually { await clock.sleepRequestCount() >= 2 }
        ...
        await clock.advance(by: .milliseconds(1))
        let restoredAtDeadline = await eventually { await light.restoreCount() == 1 }
        XCTAssertTrue(restoredAtDeadline)
    ...
    func testRecoveryHonorsOnlyRemainingUnexpiredPhysicalTerminalHold() async throws {
    ...
    func testRecoveryTreatsExpiredTerminalMetadataAsImmediateRestore() async throws {
        let applied = Date(timeIntervalSince1970: 1_700_000_000)
        let record = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.error),
            terminal: MonitoringTerminalRecovery(
                appliedAt: applied,
        ...
        let orchestrator = makeOrchestrator(
            ...
            clock: clock,
            wallNow: { applied.addingTimeInterval(13) }
        )
    ...
    func testRecoveryDoesNotRestoreWhenBulbChangesDuringRemainingTerminalHold() async throws {
        ...
        let light = RecordingLightController(
            baseline: external,
            matchResults: [.success(true), .success(false)]
        )
        let store = MemoryRecoveryStore(record: record)
        let clock = ManualClock()
        let orchestrator = makeOrchestrator(
    ...
    func testSupersededLateTerminalCommitCannotExtendOlderPhysicalHold() async throws {
        let clock = ManualClock()
        let light = RecordingLightController(attemptClock: clock)
        let store = MemoryRecoveryStore()
        await store.blockSaveCall(3)
        ...
        await XCTAssertAsyncEqual(await light.restoreCount(), 1)
        await XCTAssertAsyncEqual((await orchestrator.currentSnapshot()).state, .idle)
        await XCTAssertAsyncEqual(await light.appliedStates(), [desired(.completed), desired(.error)])
    }
    ...
    func testReconnectAfterTerminalCommitFailureReappliesCommitsThenStartsHold() async throws {
        ...
        let healthScheduled = await eventually { await clock.sleepRequestCount() >= 3 }
        XCTAssertTrue(healthScheduled)
        guard healthScheduled else {
            reconnect.cancel()
            return
        }
        await clock.advance(by: .seconds(1))
        ...
        let reapplyScheduled = await eventually { await clock.sleepRequestCount() >= 4 }
        XCTAssertTrue(reapplyScheduled)
        guard reapplyScheduled else {
            reconnect.cancel()
            return
        }
        await clock.advance(by: .seconds(1))
        let reapplied = await eventually { await light.appliedStates().count == 2 }
        XCTAssertTrue(reapplied)
        guard reapplied else {
            reconnect.cancel()
            return
        }
        let reconnectCompleted = await eventually { await completions.value() == 1 }
        XCTAssertTrue(reconnectCompleted)
        guard reconnectCompleted else {
            reconnect.cancel()
            return
        }
        await reconnect.value
        let holdScheduled = await eventually { await clock.sleepRequestCount() >= 5 }
        XCTAssertTrue(holdScheduled)
        guard holdScheduled else { return }

        await clock.advance(by: .milliseconds(7_999))
        await drainScheduledTasks()
        await XCTAssertAsyncEqual((await orchestrator.currentSnapshot()).state, .completed)
        await clock.advance(by: .milliseconds(1))
        let restored = await eventually { await light.restoreCount() == 1 }
...
final class FileMonitoringRecoveryStoreTests: XCTestCase {
    func testRecoveryArtifactsRemainBoundedAcrossRepeatedSaveAndClear() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let activeName = "monitoring-recovery-v1.json"
        ...
        for index in 0..<200 {
            let state: AgentState = index.isMultiple(of: 2) ? .working : .thinking
            let record = MonitoringRecoveryRecord(
                baseline: .testBaseline,
                lastCommand: desired(state)
            )
        ...
        XCTAssertTrue(names.isSubset(of: allowedNames), "Unexpected recovery artifacts: \(names.subtracting(allowedNames))")
        XCTAssertLessThanOrEqual(names.count, allowedNames.count)
    }

    func testUnknownFilesRemainByteForByteUnchangedDuringBoundedMaintenance() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("injected-recovery.json")
        let unknown = directory.appendingPathComponent("user-owned-note")
        let unknownBytes = Data([0x00, 0x7F, 0xFF, 0x41])
        ...
        for index in 0..<50 {
            let record = MonitoringRecoveryRecord(
                baseline: .testBaseline,
                lastCommand: desired(index.isMultiple(of: 2) ? .working : .thinking)
            )
            let revision = try await store.save(record)
        ...
        let names = Set(try FileManager.default.contentsOfDirectory(atPath: directory.path))
        XCTAssertTrue(names.isSubset(of: [
    ...
    func testLoadSelectsNewestRecoverableFixedSlotAtCrashBoundaries() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let active = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let previous = directory.appendingPathComponent(".monitoring-recovery.previous")
        let tombstone = directory.appendingPathComponent(".monitoring-recovery.tombstone")
        let original = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.thinking)
        )
        let replacement = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.working)
        )
        try writePrivateRecord(original, to: previous)

        let interruptedBeforeInstall = FileMonitoringRecoveryStore(url: active)
        await XCTAssertAsyncEqual((try await interruptedBeforeInstall.load())?.record, original)

        try writePrivateRecord(replacement, to: active)
        let interruptedAfterInstall = FileMonitoringRecoveryStore(url: active)
        await XCTAssertAsyncEqual((try await interruptedAfterInstall.load())?.record, replacement)

        try FileManager.default.removeItem(at: active)
        try writePrivateRecord(replacement, to: tombstone)
        let interruptedClear = FileMonitoringRecoveryStore(url: active)
        await XCTAssertAsyncNil(try await interruptedClear.load())
    }

    func testTwoStoreInstancesSerializeHundredsOfSavesAndRejectCrossInstanceStaleRevision() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let firstStore = FileMonitoringRecoveryStore(url: url)
        let secondStore = FileMonitoringRecoveryStore(url: url)
        let staleRecord = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.thinking)
        )
        let staleRevision = try await firstStore.save(staleRecord)
        let winner = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.working)
        )
        _ = try await secondStore.save(winner)

        await XCTAssertThrowsSpecificError(MonitoringRecoveryStoreError.concurrentModification) {
            try await firstStore.clear(
                expecting: StoredMonitoringRecovery(record: staleRecord, revision: staleRevision)
            )
        }
            ...
            for index in 0..<200 {
                group.addTask {
                    ...
                    if index.isMultiple(of: 2) {
                        _ = try await firstStore.save(record)
                    } else {
                        _ = try await secondStore.save(record)
                    }
                }
            }
        ...
        let loaded = try await FileMonitoringRecoveryStore(url: url).load()
    ...
    func testLockReplacementBeforeStageWriteCannotSplitDirectoryCoordination() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let lockURL = directory.appendingPathComponent(".monitoring-recovery.lock")
        let original = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.thinking)
        )
        _ = try await FileMonitoringRecoveryStore(url: url).save(original)
        let firstOperations = FaultInjectingRecoveryPOSIXOperations()
        let firstBlocked = SynchronousTestGate()
        firstOperations.beforeTruncate = {
            try replaceInode(at: lockURL, bytes: Data())
            firstBlocked.enterAndWait()
        }
        let firstStore = FileMonitoringRecoveryStore(url: url, operations: firstOperations)
        let first = Task {
            try await firstStore.save(
                MonitoringRecoveryRecord(
                    baseline: .testBaseline,
                    lastCommand: desired(.working)
                )
            )
        }
        defer { firstBlocked.release() }
        XCTAssertTrue(firstBlocked.waitForEntries(1))

        let secondOperations = FaultInjectingRecoveryPOSIXOperations()
        let secondAttemptedLock = SynchronousTestGate()
        secondOperations.beforeLockExclusive = { secondAttemptedLock.signal() }
        let secondStore = FileMonitoringRecoveryStore(url: url, operations: secondOperations)
        let winner = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.error)
        )
        let second = Task { try await secondStore.save(winner) }
        XCTAssertTrue(secondAttemptedLock.waitForEntries(1))
        XCTAssertEqual(try decodeRecoveryRecord(at: url), original)

        firstBlocked.release()
        await XCTAssertTaskThrowsSpecificError(first, MonitoringRecoveryStoreError.concurrentModification)
        _ = try await second.value

        await XCTAssertAsyncEqual((try await secondStore.load())?.record, winner)
    }

    func testLockReplacementDuringRollbackCannotLetNewCommitBeRolledOver() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let lockURL = directory.appendingPathComponent(".monitoring-recovery.lock")
        ...
        _ = try await FileMonitoringRecoveryStore(url: url).save(original)
        let firstOperations = FaultInjectingRecoveryPOSIXOperations()
        firstOperations.failNextDirectorySync = true
        let rollbackBlocked = SynchronousTestGate()
        firstOperations.beforeActualSwap = {
            try replaceInode(at: lockURL, bytes: Data())
            rollbackBlocked.enterAndWait()
        }
        let firstStore = FileMonitoringRecoveryStore(url: url, operations: firstOperations)
        let first = Task {
            try await firstStore.save(
        ...
        defer { rollbackBlocked.release() }
        XCTAssertTrue(rollbackBlocked.waitForEntries(1))
        ...
        rollbackBlocked.release()
        await XCTAssertTaskThrowsSpecificError(first, MonitoringRecoveryStoreError.ioFailure)
        _ = try await second.value

        await XCTAssertAsyncEqual((try await secondStore.load())?.record, winner)
    }

    func testSaveRejectsActiveReplacementAfterDirectorySyncWithoutResurrectingPrevious() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let previousURL = directory.appendingPathComponent(".monitoring-recovery.previous")
        ...
        let attempted = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.working)
        )
        let concurrent = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.error)
        )
        ...
        await XCTAssertThrowsSpecificError(MonitoringRecoveryStoreError.concurrentModification) {
            _ = try await store.save(attempted)
        }

        XCTAssertEqual(try decodeRecoveryRecord(at: url), concurrent)
        XCTAssertEqual(try decodeRecoveryRecord(at: previousURL), original)
        await XCTAssertAsyncEqual(
            (try await FileMonitoringRecoveryStore(url: url).load())?.record,
            concurrent
        )
    }

    func testClearRejectsTombstoneRemovalAfterDirectorySyncWithoutResurrectingPrevious() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let tombstoneURL = directory.appendingPathComponent(".monitoring-recovery.tombstone")
        let operations = FaultInjectingRecoveryPOSIXOperations()
        let store = FileMonitoringRecoveryStore(url: url, operations: operations)
        ...
        let current = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.working)
        )
        let revision = try await store.save(current)
        operations.afterNextDirectorySync = {
        ...
        await XCTAssertThrowsSpecificError(MonitoringRecoveryStoreError.concurrentModification) {
            try await store.clear(
                expecting: StoredMonitoringRecovery(record: current, revision: revision)
            )
        }

        await XCTAssertAsyncNil(try await FileMonitoringRecoveryStore(url: url).load())
    }

    func testCancelledClearRollbackNeverReplacesConcurrentDestination() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let tombstoneURL = directory.appendingPathComponent(".monitoring-recovery.tombstone")
        ...
        operations.afterNextRenameExclusive = { _, destination in
            guard destination == ".monitoring-recovery.tombstone" else { return }
            try writePrivateRecoveryRecord(concurrent, to: url)
            withUnsafeCurrentTask { $0?.cancel() }
        }

        let clear = Task {
            try await store.clear(
                expecting: StoredMonitoringRecovery(record: original, revision: revision)
            )
        }
        await XCTAssertTaskIsCancelled(clear)

        XCTAssertEqual(try decodeRecoveryRecord(at: url), concurrent)
        XCTAssertEqual(try decodeRecoveryRecord(at: tombstoneURL), original)
    }

    func testClearMismatchRollbackRetainsConcurrentDestinationAndRecoverySlot() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let tombstoneURL = directory.appendingPathComponent(".monitoring-recovery.tombstone")
        ...
        let concurrentDestination = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.working)
        )
        let retainedRecovery = MonitoringRecoveryRecord(
            baseline: .testBaseline,
            lastCommand: desired(.error)
        )
        let revision = try await store.save(original)
        operations.afterNextRenameExclusive = { _, destination in
            guard destination == ".monitoring-recovery.tombstone" else { return }
            try replaceInode(at: tombstoneURL, bytes: try encoded(retainedRecovery))
            try writePrivateRecoveryRecord(concurrentDestination, to: url)
        }
        ...
        XCTAssertEqual(try decodeRecoveryRecord(at: url), concurrentDestination)
        XCTAssertEqual(try decodeRecoveryRecord(at: tombstoneURL), retainedRecovery)
    }

    func testKnownPreviousSlotSymlinkIsRejectedWithoutTouchingItsTarget() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let target = directory.appendingPathComponent("unknown-target")
        let targetBytes = Data("unchanged".utf8)
        try targetBytes.write(to: target)
        let previous = directory.appendingPathComponent(".monitoring-recovery.previous")
        XCTAssertEqual(symlink(target.path, previous.path), 0)
        let store = FileMonitoringRecoveryStore(url: url)
    ...
    func testNewLockFileIsForcedToMode0600UnderRestrictiveUmask() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let lockURL = directory.appendingPathComponent(".monitoring-recovery.lock")
        ...
        _ = try await FileMonitoringRecoveryStore(url: url).save(
            MonitoringRecoveryRecord(baseline: .testBaseline)
        )

        var metadata = stat()
        XCTAssertEqual(lstat(lockURL.path, &metadata), 0)
        XCTAssertEqual(metadata.st_mode & 0o777, 0o600)
    }

    func testPreexistingUnsafeLockModeIsRejectedWithoutRepair() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let lockURL = directory.appendingPathComponent(".monitoring-recovery.lock")
    ...
    func testRepeatedFailedStagesRemainBoundedAndNextSuccessAbsorbsTheStage() async throws {
        let directory = try temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let url = directory.appendingPathComponent("monitoring-recovery-v1.json")
        let operations = FaultInjectingRecoveryPOSIXOperations()
        ...
        let names = Set(try FileManager.default.contentsOfDirectory(atPath: directory.path))
        XCTAssertFalse(names.contains(".monitoring-recovery.stage"))
        XCTAssertTrue(names.isSubset(of: [
            "monitoring-recovery-v1.json",
            ".monitoring-recovery.previous",
            ".monitoring-recovery.lock",
    ...
    func testTemporaryCleanupRetainsAuthenticatedArtifactWithoutPathUnlink() async throws {
        ...
        let artifacts = try recoveryArtifactURLs(in: directory)
    ...
    func testConditionalClearDirectorySyncFailurePreservesQuarantinedRecord() async throws {
        ...
        try await store.clear(expecting: expected)
        let retained = try recoveryArtifactURLs(in: directory)
    ...
    func testSaveParentReplacementAfterCommitSyncPreservesDisplacedRecord() async throws {
        ...
        let artifacts = try recoveryArtifactURLs(in: movedParent)
        let records = try artifacts.map { try JSONDecoder().decode(MonitoringRecoveryRecord.self, from: Data(contentsOf: $0)) }
        XCTAssertEqual(records.count, 2)
        XCTAssertTrue(records.contains(original))
        XCTAssertTrue(records.contains(replacement))
    ...
    private func recoveryArtifactURLs(in directory: URL) throws -> [URL] {
        try FileManager.default
            ...
            .filter { $0.lastPathComponent != ".monitoring-recovery.lock" }
    }

    private func writePrivateRecord(_ record: MonitoringRecoveryRecord, to url: URL) throws {
        try encoded(record).write(to: url)
        try FileManager.default.setAttributes(
...
private final class FaultInjectingRecoveryPOSIXOperations: MonitoringRecoveryPOSIXOperations, @unchecked Sendable {
    ...
    var beforeLockExclusive: (() -> Void)?
    var beforeTruncate: (() throws -> Void)?
    var beforeActualSwap: (() throws -> Void)?
    ...
    func openOrCreateWritable(at directory: Int32, name: String, mode: mode_t) throws -> Int32 {
        let descriptor = try base.openOrCreateWritable(at: directory, name: name, mode: mode)
        _ = locked { openFileDescriptors.insert(descriptor) }
        return descriptor
    }
    ...
    func metadata(at directory: Int32, name: String) throws -> MonitoringRecoveryFileMetadata? {
        try base.metadata(at: directory, name: name)
    }
    ...
    func truncate(_ descriptor: Int32) throws {
        let hook = locked { () -> (() throws -> Void)? in
            defer { beforeTruncate = nil }
            return beforeTruncate
        }
        try hook?()
        try base.truncate(descriptor)
    }
    ...
    func lockExclusive(_ descriptor: Int32) throws {
        let beforeHook = locked { beforeLockExclusive }
        beforeHook?()
        try base.lockExclusive(descriptor)
    }
    ...
    func swap(at directory: Int32, _ first: String, _ second: String) throws {
        let actualSwapHook = locked { () -> (() throws -> Void)? in
            defer { beforeActualSwap = nil }
            return beforeActualSwap
        }
        try actualSwapHook?()
        let hook = locked { () -> (() throws -> Void)? in
    ...
    func renameReplacing(at directory: Int32, from: String, to: String) throws {
        let swapHook = locked { () -> (() throws -> Void)? in
            defer { beforeSwap = nil }
            return beforeSwap
        }
        try swapHook?()
        let renameHook = locked { () -> (() throws -> Void)? in
            defer { beforeRenameExclusive = nil }
            return beforeRenameExclusive
        }
        try renameHook?()
        ...
        if shouldFail { throw MonitoringRecoveryPOSIXError.system(EIO) }
        try base.renameReplacing(at: directory, from: from, to: to)
        let afterHook = locked { () -> ((String, String) throws -> Void)? in
            defer { afterNextRenameExclusive = nil }
            return afterNextRenameExclusive
        }
...
private final class SynchronousTestGate: @unchecked Sendable {
...
private func writePrivateRecoveryRecord(_ record: MonitoringRecoveryRecord, to url: URL) throws {
    try encoded(record).write(to: url)
    try FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: url.path)
}

private func decodeRecoveryRecord(at url: URL) throws -> MonitoringRecoveryRecord {
    try JSONDecoder().decode(MonitoringRecoveryRecord.self, from: Data(contentsOf: url))
}
...
private func XCTAssertTaskThrowsSpecificError<T: Sendable, E: Error & Equatable>(
    _ task: Task<T, Error>,
    _ expected: E,
    file: StaticString = #filePath,
    line: UInt = #line
